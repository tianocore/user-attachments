  ...
  )
{
  ...
  if (OldCpuMpData == NULL) {
    ...
    BspNumber = OldCpuMpData->BspNumber;
  }
  ...
  BufferSize += VolatileRegisters.Idtr.Limit + 1 + 4;
  ...
  DEBUG ((DEBUG_ERROR, "AP IDT Base = %p, Limit = %x\n", ApIdtBase, VolatileRegisters.Idtr.Limit));
  CpuMpData        = (CPU_MP_DATA *) (ApIdtBase + VolatileRegisters.Idtr.Limit + 1 + 4); // make AP IDTR.LIMIT extra 4 bytes.
  CpuMpData->Signature = SIGNATURE_32 ('M', 'P', 'D', 'A');
  ...
  CpuMpData->BspNumber        = (UINT32) BspNumber;
  ...
  VolatileRegisters.Idtr.Limit += 4;
  //
  // Don't pass BSP's TR to APs to avoid AP init failure.
  //
  VolatileRegisters.Tr = 0;
  CopyMem (&CpuMpData->CpuData[BspNumber].VolatileRegisters, &VolatileRegisters, sizeof (VolatileRegisters));
  //
  // Set BSP basic information
  //
  InitializeApData (CpuMpData, BspNumber, 0, CpuMpData->Buffer + ApStackSize);
  //
  ...
  )
{
  ...
  ApicBaseMsr.Uint64 = AsmReadMsr64 (MSR_IA32_APIC_BASE);
  DEBUG ((DEBUG_ERROR, "SwitchBsp: Bsp Flag = %d\n", ApicBaseMsr.Bits.BSP));
  ProgramVirtualWireMode ();
  ...
  )
{
  ...
  CpuMpData = GetCpuMpData ();
  ASSERT (CpuMpData->Signature == SIGNATURE_32 ('M', 'P', 'D', 'A'));
