...
gen_signed_authvar()
{
	...
	./sign-efi-sig-list -g "$(< GUID.txt)" -k $signer.key -c $signer.crt -sha$digest $var $signed.esl $signed.auth
	./ftoarr --infile $signed.auth --outfile $signed.h
}
...
gen_authvar()
{
	...
	base_ver=$var
	base_ver=${base_ver}_$rsa
	base_ver=${base_ver}_$digest
	...
	update_ver=$var
	update_ver="${update_ver}New_$rsa"
	update_ver="${update_ver}_$digest"
