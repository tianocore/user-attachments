...
#define PRINT_MESSAGE_BUFFER_SIZE  1024
  ...
  )
{
  ...
  UNIT_TEST_SUITE      *Suite;
  UNIT_TEST_FRAMEWORK  *Framework;
  UINTN                UnitTestLogSize = 0;
  UINTN                i;
  CHAR8                *TempLogPtr = NULL;
  ...
  if (UnitTest->Log != NULL) {
    ...
    UnitTestLogSize = strlen (UnitTest->Log);
    TempLogPtr = UnitTest->Log;
    for (i = 0; i < UnitTestLogSize; i += (PRINT_MESSAGE_BUFFER_SIZE - 1)) {
      print_message ("%s", TempLogPtr);
      TempLogPtr += (PRINT_MESSAGE_BUFFER_SIZE - 1);
    }
