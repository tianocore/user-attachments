  ...
  )
{
  ...
  for (Index = 1; Index <= 16; Index++) {  // vz-bug-1 - Start is an array of 17, but Index == 16 and Index + 1 means line 176 is one beyond size of array e.g., Start[17].  only persmissible range is Start[0] to Start[16]
  ...
  for (Char = 0; Char < NumOfChar; Char++) {
    ...
    if (Len <= TableBits) {
      ...
      while (Index != 0) {
        if (*Pointer == 0 && Avail < (2 * NC - 1)) {  // vz-bug-2 - need to check bounds of Avail
        ...
        if (*Pointer < (2 * NC - 1)) {
          if ((Index3 & Mask) != 0) {
            Pointer = &Sd->mRight[*Pointer];  // vz-bug-3 - need to bounds check Pointer
          } else {
            Pointer = &Sd->mLeft[*Pointer];    // vz-bug-4 - need to bounds check Pointer
          }
        }
  ...
  )
{
  ...
  //
  Number = (UINT16) GetBits (Sd, nbit);  // vz-bug-5 - needs propoer bounds checking
  ...
  while (Index < Number && Index < NPT) {
    ...
    if (Index == Special) {
      ...
      while ((INT16) (--CharC) >= 0 && Index < NPT) {  // vz-bug-6 - needs to check for underflow
        Sd->mPTLen[Index++] = 0;
      }
    }
  }
  ...
  )
{
  ...
  for (;;) {
    ...
    if (CharC < 256) {
      ...
      //
      DataIdx     = Sd->mOutBuf - DecodeP (Sd) - 1;  // vz-bug-7 - need to check for underflow
